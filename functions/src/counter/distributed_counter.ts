...
import { WriteResult, DocumentReference, Transaction } from '@google-cloud/firestore';
 ...
 */
export async function createCounter(name: string): Promise < WriteResult[] > {
    const ref: DocumentReference = db.collection(COUNTER_COLLECTION).doc(name);
 ...
 */
export async function incrementCounter(name: string, transaction: Transaction): Promise < Transaction > {
    const ref: DocumentReference = db.collection(COUNTER_COLLECTION).doc(name);
    // Select a shard of the counter at random
    const randomShardID: string = Math.floor(Math.random() * NUMBER_OF_SHARDS).toString();
    const shardRef: DocumentReference = ref.collection('shards').doc(randomShardID);
